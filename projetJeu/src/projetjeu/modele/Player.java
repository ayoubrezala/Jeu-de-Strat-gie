 ...
 * The game actors.
 */
public class Player extends Clonable {
     ...
     * Player's class.
    */
    public Player(PrincipalGrid g, String name, String classname) {
        super(0,0);
        ...
        this.view = new GridProxy(g,this);
        this.selected = false;
        this.walkable=false;
    }
    ...
    public void move(Direction d) {
        if (this.possibleMoves().contains(d)) {
            this.view.setTileAt(new FreeCase(x,y));
            this.x += d.x();
            this.y += d.y();

            try {
                ((Pastille)this.view.getTileAt(x,y)).boost(this);
    ...
    public ArrayList<ArrayList<Integer>> visibleTiles(){
        ...
        while(c<this.visionSize){
            if(this.x+c>view.getModel().getWidth()){
                ...
                if(!(view.getModel().getTileAt(this.x+c, this.y) instanceof FreeCase)){
                    break;
                }
                else{
        ...
        while(c<this.visionSize){
            if(this.y+c>view.getModel().getGrid().length/view.getModel().getWidth()){
                ...
                if(!(view.getModel().getTileAt(this.x, this.y+c) instanceof FreeCase)){
                    break;
                }
                else{
                    ArrayList<Integer> pos = new ArrayList<>(2);
    ...
    public ArrayList<ArrayList<Integer>> visiblesTiles(){
        ...
        for(int vY = 0; vY<mapSizeY; vY++){
            for(int vX = 0; vX<mapSizeX; vX++){
                ...
                Clonable t = view.getModel().getTileAt(vX, vY);
                boolean isWall = t instanceof Mur;
                    ...
                    for(int narrowvX=-1; narrowvX<=1; narrowvX+=1){
                        for(int narrowvY=-(1); narrowvY<=1; narrowvY+=1){
                            try{
                                ...
                                narrowContained = narrowContained || (visibles.contains(narrow) && !(view.getModel().getTileAt(vX+narrowvX, vY+narrowvY) instanceof Mur));
                            }catch(Exception e){ }
                        }
                    }
    ...
    public void takeDamage(int damage) {
        if (!(this.shield_up)) {
            ...
            if (this.life<=0) {
                this.energy=0;
                this.view.setTileAt(new FreeCase(this.x,this.y));
    ...
    */
    ...
    /** Plants an explosive on the parameter tile.
      ...
      * The planting site.
    */
    public void plant(Mine explosive, Clonable t) {
        if (t!=null) {
    ...
    public void fire(Direction d) {
        ...
        for (Weapon w : this.loadout.keySet()) {
            if (w instanceof Fuisil) {
                Fuisil fire_rifle = new Fuisil(this);//new Fuisil just for firing
